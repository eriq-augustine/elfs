...
func ReadUsers(users map[user.Id]*user.User, reader *cipherio.CipherReader) error {
   err := ReadUsersWithDecoder(users, json.NewDecoder(reader));
   if (err != nil) {
...
func ReadUsersWithDecoder(users map[user.Id]*user.User, decoder *json.Decoder) error {
   size, err := decodeMetadata(decoder);
   if (err != nil) {
      return errors.Wrap(err, "Could not decode user metadata.");
   }
...
func WriteUsers(users map[user.Id]*user.User, writer *cipherio.CipherWriter) error {
   var encoder *json.Encoder = json.NewEncoder(writer);

   err := encodeMetadata(encoder, len(users));
   if (err != nil) {
